...
import 'package:flutter_tetris/riverpod_version/state.dart';

import '../models.dart';

class RiverpodVersionController {
  late RiverpodVersionState state;
  late WidgetRef ref;

  RiverpodVersionController(this.ref) {
    state = RiverpodVersionState(ref);
  }

  bool _leftInProcess = false, _rightInProcess = false, _downInProcess = false;

  void startAutoDrop(WidgetRef ref) async {
    state.looping = true;
    while (true) {
  ...
  void rotate() {
    ...
    for (BlockStatus block in tmpShape.blocks) {
      if (block.y >= 20 ||
          ...
          block.x >= 10 ||
          (block.y >= 0 &&
              ref.read(state.map[block.y][block.x]) == BlockColor.black)) {
        drawShape();
        return;
      }
    }
  ...
  void hideShape() {
    for (BlockStatus value in state.shape.blocks) {
      if (value.y >= 0 && value.y < 20 && value.x >= 0 && value.x < 10) {
        ref.read(state.map[value.y][value.x].state).state = BlockColor.white;
      }
    }
  }
  ...
  void moveToBottom() async {
    ...
    if (state.paused) {
      state.paused = false;
      if (!state.looping) startAutoDrop(ref);
      return;
    }
    hideShape();
    while (canMoveDown) {
  ...
  Future shineLines(List<int> lines) async {
    ...
    for (var value in lines) {
      for (var value1 in (state.map[value])) {
        ref.read(value1.state).state = BlockColor.red;
      }
    }
    for (int i = 0; i < 7; i++) {
      await Future.delayed(Duration(milliseconds: 70));
  ...
  void moveLineDownward(int y, int num) {
    ...
    for (int i = 0; i < 10; i++) {
      ref.read(state.map[y + num][i].state).state = ref.read(tmpLineY[i]);
    }
  }
  ...
  bool isLineCompleted(List<StateProvider<BlockColor>> line) {
    for (var value in line) {
      if (ref.read(value) == BlockColor.white) {
        return false;
      }
    }
    return true;
  ...
  int getFirstBlockBelowShapeInColumnX(int x) {
    for (int i = state.shape.bottomOnX(x) + 1; i < 20; i++) {
      if (ref.read(state.map[i][x]) == BlockColor.black) {
        return i;
      }
    }
    return 20;
  ...
  int getFirstActivatedBlockOnRight(int x, int y) {
    for (int i = x; i < 10; i++) {
      if (y >= 0 && ref.read(state.map[y][i]) == BlockColor.black) {
        return i;
      }
    }
    return 10;
